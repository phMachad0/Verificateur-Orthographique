...
int main(int ac, char**av) {
	...
	for (int k=nbDicoWords; k>1; k-=nbDicoWords/100) {
		hashlset_t hash=hashlset_new(100000, mystring_hashage, mystring_fprintf, mystring_delete, mystring_equal);
		f=fopen(av[1],"r");
        ...
        taille = hashlset_memory_usage(hash);
        ...
        while (lecture(f,mot)) {
			if (mot[0] != 0) {
				if (hashlset_find_key(mot,hash) == 0) {
					noFound++;
